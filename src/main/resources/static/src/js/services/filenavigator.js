    ...
    function fileNavigator($http, $q, fileManagerConfig, Item, publicId) {
        ...
        FileNavigator.prototype.list = function () {
            ...
            var data = {
                params: {
                    mode: 'list',
                    onlyFolders: fileManagerConfig.onlyFolders,
                    path: '/' + path,
                    publicId: publicId
                }
            };
        ...
        FileNavigator.prototype.refresh = function () {
            ...
            return self.list().then(function (data) {
                ...
                self.deletedExpiredFileList = (data.result || []).map(function (file) {
                    if (file.deleted === true || file.expired === true) {
                        return new Item(file, self.currentPath);
                    }
                });
                self.fileList = (data.result || []).map(function (file) {
                    if (file.deleted === false && file.expired === false) {
                        return new Item(file, self.currentPath);
                    }
                });
                self.buildTree(path);
        ...
        FileNavigator.prototype.buildTree = function (path) {
            function recursive(parent, item, path) {
                ...
                //var displayName = (typeof item.model.displayName === 'undefined') ? '/' : item.model.displayName;
                ...
                if (parent.name !== path) {
                    ...
                    //parent.nodes.push({item: item, name: absName, displayName: displayName, nodes: []});
