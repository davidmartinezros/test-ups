...
public class UploadServlet extends HttpServlet {
     ...
     */
    public static final String UPLOAD_DIR = "/Users/jeremyhulick/projects";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        response.setHeader("Access-Control-Allow-Origin", "http://unika.localdomain");
        ...
        String archiveFinished = info.checkIfUploadFinished();
        if (archiveFinished != null) { // Check if all chunks uploaded, and
            // change filename
    ...
    private FlowInfo getFlowInfo(HttpServletRequest request) throws ServletException {
        ...
        long FlowTotalSize = HttpUtils.toLong(request.getParameter("flowTotalSize"), -1); //flowTotalChunks
        ...
        FlowInfo info = storage.get(FlowChunkSize, FlowTotalSize, FlowIdentifier, FlowFilename, FlowRelativePath, FlowFilePath);
        if (!info.valid()) {
            storage.remove(info);
